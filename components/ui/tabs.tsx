...
function TabsList({ className, ...props }: React.ComponentProps<typeof TabsPrimitive.List>) {
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={cn(
        "bg-(--secondary-color) text-(--secondary-text-color) inline-flex h-9 w-fit items-center justify-center rounded-lg p-[3px] gap-1",
        className
      )}
      {...props}
    />
